...
class BaseModel(InitialParameterRepresenterMixIn, LightningModule, TupleOutputMixIn):
    ...
    def plot_prediction(
        ...
        for y_raw, y_hat, y_quantile, encoder_target, decoder_target in zip(
            ...
            n_pred = y_hat.shape[0]
            x_obs = np.arange(-(y.shape[0] - n_pred), 1) # changed by Ojas; to plot even the 0th point
            ...
            if len(x_obs) > 0:
                if len(x_obs) > 1:
                    ...
                    plotter = ax.scatter
                plotter(x_obs, y[:-n_pred + 1], label="observed", c=obs_color) # changed by: Ojas; to plot even the 0th point
            if len(x_pred) > 1:
                plotter = ax.plot
            else:
                plotter = ax.scatter
